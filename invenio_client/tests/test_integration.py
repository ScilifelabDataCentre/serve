...
def test_complete_workflow(invenio_client, sample_schema_org_data):
    ...
    search_response = {
        "hits": {"total": 1, "hits": [{"id": "published-123"}]}
    }
    ...
    updated_draft_response = {
        "id": "draft-from-published-123",
        "metadata": {"title": "Updated Draft Title"}
    }
    ...
    updated_new_version_response = {
        "id": "new-version-draft-123",
        "metadata": {"title": "New Version Draft - Updated"}
    }
    ...
    # 5. Update draft
    updated_draft = invenio_client.update_draft(
        record_id=edit_draft["id"],
        metadata={"title": "Updated Draft Title"}
    )
...
def test_metadata_transformation_integration(sample_schema_org_data):
    ...
    required_fields = ["title", "description", "publication_date", "publisher", 
                      "resource_type", "creators", "rights", "additional_descriptions", "subjects"]
