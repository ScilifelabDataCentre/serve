...
from doi_minting.clients.invenio_client.exceptions import (
...
class MockInvenioClient:
    def create_draft(self, record_data: dict[str, Any]) -> dict[str, Any]:
        return self._mock_record("mock-record-id", is_published=False)

    def reserve_doi(self, draft_id: str) -> dict[str, Any]:
        return self._mock_record("mock-record-id", is_published=False)

    def publish_draft(self, draft_id: str) -> dict[str, Any]:
        return self._mock_record("mock-record-id", is_published=True)

    def create_new_version(self, record_id: str) -> dict[str, Any]:
        return self._mock_record("mock-record-id", is_published=False)

    def get_draft(self, version_id: str) -> dict[str, Any]:
        if version_id == "mock-record-id":
            return self._mock_record("mock-record-id", is_published=False)

        raise InvenioClientRequestError(f"Draft not found: {version_id}")
        ...
        return {
            ...
            "metadata": {
                ...
                "additional_descriptions": [
                    {
                        "description": (
                            "URL of latest version deployment: https://mock-app.serve.scilifelab.se; "
