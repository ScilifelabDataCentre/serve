...
class VocabularyMemoryService:
    ...
    def _load_vocabulary_data(self):
        ...
        vocab_dir = getattr(settings, 'VOCABULARY_DATA_PATH', 
                           os.path.join(settings.BASE_DIR, 'common', 'data', 'invernio_keywords'))
        ...
        logger.info(f"Loaded vocabulary: {len(self.term_metadata)} terms, "
                   f"{len(self.autocomplete_data)} prefixes")
