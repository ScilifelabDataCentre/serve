...
def generate_form(aset, project, app, user, appinstance=[]):
    ...
    if "flavor" in aset:
        ...
        form["current_flavor"] = Flavor.objects.filter(project=project, id=appinstance.flavor.id) if appinstance else None
        form["flavors"] = Flavor.objects.filter(project=project).exclude(id=appinstance.flavor.id) if appinstance else Flavor.objects.filter(project=project)
