...
class DeleteAppViewTestCase(TestCase):
    ...
    def test_public_app_cannot_be_deleted(self):
        """Test that public apps with DOIs cannot be deleted regardless of user_can_delete setting"""
        c = Client()

        response = c.post("/accounts/login/", {"username": test_user["email"], "password": test_user["password"]})
        self.assertEqual(response.status_code, 302)

        # Create a public app instance with DOI for this test
        from apps.models.app_types.dash import DashInstance

        subdomain = Subdomain.objects.create(subdomain="test_public")
        k8s_user_app_status = K8sUserAppStatus.objects.create()

        # Get dash app (which supports DOI via SocialMixin)
        dash_app = Apps.objects.filter(slug="dashapp").first()
        if not dash_app:
            dash_app = Apps.objects.create(slug="dashapp", name="Dash App", user_can_delete=True)

        public_app_instance = DashInstance.objects.create(
            access="public",
            app_doi="10.1000/test.doi",  # Add DOI to trigger protection
            owner=self.user,
        ...
        # Even with user_can_delete=True, public apps with DOIs should not be deletable
        dash_app.user_can_delete = True
        dash_app.save()
        ...
        # Should return 403 Forbidden for public apps with DOIs
        self.assertEqual(response.status_code, 403)
