...
def test_is_available_for_available_subdomains():
    ...
    candidate = SubdomainCandidateName("test-name-is-unique", 1)
    assert candidate.is_available()

    candidate = SubdomainCandidateName("test-name-999-is-unique", 1)
    assert candidate.is_available()
...
def test_is_available_for_unavailable_subdomains():
    ...
    candidate = SubdomainCandidateName("serve", 1)
...
def test_is_valid_for_valid_names(name):
    ...
    candidate = SubdomainCandidateName(name, 1)
...
def test_validate_subdomain():
    ...
    candidate = SubdomainCandidateName("test-subdomain", 1)
    candidate.validate_subdomain()

    # Test with invalid subdomain (too short)
    candidate = SubdomainCandidateName("a", 1)
