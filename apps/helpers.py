...
HELP_MESSAGE_MAP = {
    ...
    "source_code_url": "Provide a URL where the full source code of your app can be accessed (for example, to a GitHub repository or a Figshare or Zenodo entry).",
    "flavor": "Hardware allocation for your app. Only one option is available by default. If your app requires more hardware resources, get in touch with us (serve@scilifelab.se) with a request.",
    "image": "Docker Image for the app uploaded to DockerHub or GitHub. Each version of your app should have a unique tag.",
    "path": "Specify the path inside the container that you want to be persistent (path to database or similar). If you follow our guide to build the container, then please include the username in the path as well.",
    "port": "Port that the docker container exposes and the application runs on. This should be an integer between 3000-9999.",
}
